...
const FLUX_FROM = "var(--flux-from, #00A6FF)";
...
function FluxLabel({ label, reduced, className }: FluxLabelProps) {
  const base = cn(
    "absolute inset-0 flex items-center justify-center text-center font-semibold tracking-tight text-adam-text-secondary text-lg",
    className,
  );
...
export function ProgressiveFluxLoader({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative h-4 w-full overflow-hidden rounded-full bg-adam-neutral-800 shadow-[inset_0_2px_3px_rgba(0,0,0,0.45),inset_0_-1px_2px_rgba(255,255,255,0.05)]",
          barClassName,
        )}
        role="progressbar"
        aria-valuemin={0}
